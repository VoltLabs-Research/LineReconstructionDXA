...
public:
    ...
    [[nodiscard]] bool isFiniteCell(CellHandle cell) const{
    ...
    [[nodiscard]] std::array<VertexHandle, 4> cellVertices(CellHandle cell) const{
        return {
            cellVertex(cell, 0),
            cellVertex(cell, 1),
            cellVertex(cell, 2),
    ...
    [[nodiscard]] int inputPointIndex(VertexHandle vertex) const{
		return vertexIndex(vertex);
	}

    [[nodiscard]] int findInputPointInCell(CellHandle cell, int atomIndex) const{
        for(int localVertex = 0; localVertex < 4; ++localVertex){
            if(inputPointIndex(cellVertex(cell, localVertex)) == atomIndex){
                return localVertex;
            }
        }
        return -1;
